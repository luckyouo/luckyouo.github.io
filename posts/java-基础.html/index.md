...
Java 底层不存在多维数组，多维数组实质是数组元素为数组的数组，所以内部数组长度可以不相等。

```java
int[][] arr = {{3,2,1}, {2, 1}};
```

由于数组元素为数组，所以可以对数组元素进行排序，即行内排序
...
也可以将数组视作为一个元素，对外层数组进行排序，即行间排序，**此时需注意内层数组长度大小**

```java
int[][] arr = {{3,2,1}, {2, 1}};
Arrays.sort(arr, Comparator.comparingInt(o -> o[1])); // 若使用o[2]，则会抛出数组越界异常
```
